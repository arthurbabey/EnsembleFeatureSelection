...
def compare_performance(selected_features_baseline, selected_features_pipeline, data):
    ...
    accuracy_pipeline, precision_pipeline, recall_pipeline, f1_pipeline, roc_auc_pipeline = [], [], [], [], []
    accuracy_baseline, precision_baseline, recall_baseline, f1_baseline, roc_auc_baseline = [], [], [], [], []

    for features_baseline, features_pipeline in zip(selected_features_baseline, selected_features_pipeline):
        # pipeline
        ...
        X_train, X_test, y_train, y_test = train_test_split(X_baseline, y, test_size=0.2)
        classifier.fit(X_train, y_train)
        ...
        roc_auc_pipeline.append(roc_auc_score(label_binarize(y_test, classes=[0, 1]), label_binarize(y_pred_pipeline, classes=[0, 1])))
        ...
        roc_auc_baseline.append(roc_auc_score(label_binarize(y_test, classes=[0, 1]), label_binarize(y_pred_baseline, classes=[0, 1])))
    ...
    print(f"""Average Performance Metrics using features selected by pipeline:
...
def run_feature_selection(data, method, num_runs=5, train_size=0.8, num_features_to_select=10):
    selected_features_list = []

    for _ in range(num_runs):
        # Randomly split the data
        _, data_subset, _, _ = train_test_split(data, train_size=train_size, stratify=data['target'], random_state=None)
        ...
        if method == 'pipeline':
            classifier = 'RF'
            fs_methods = [feature_selection_infogain, feature_selection_xgboost, feature_selection_random_forest,
                          feature_selection_chi2]
            merging_strategy = merging_strategy_union_of_pairwise_intersections
            num_repeats = 3
            pipeline = FeatureSelectionPipeline(data_subset, fs_methods, merging_strategy, classifier, num_repeats)
            selected_features, _, _ = pipeline.iterate_pipeline()

        elif method == 'baseline':
            baseline_feature_selection = feature_selection_chi2
            _, baseline_idx = baseline_feature_selection(X=X_subset, y=y_subset,
...
def compare_robustness(baseline_selected_features, pipeline_selected_features):
    ...
    for baseline_features, pipeline_features in zip(baseline_selected_features, pipeline_selected_features):
...
if __name__ == "main":
    ...
    data = pd.read_csv('breast_cancer_data/Breast_GSE45827.csv').rename(columns={'type': 'target'}).drop(columns='samples')
    ...
    pipeline_selected_features = run_feature_selection(data, method='pipeline', num_runs=5, train_size=0.8,
                                                       num_features_to_select=10)
    baseline_selected_features = run_feature_selection(data, method='baseline', num_runs=5, train_size=0.8,
