        ...
        if hasattr(clf, "predict_proba"):
            ...
            if num_classes == 2:
                ...
                y_proba = clf.predict_proba(X_test)[
                    :, 1
                ]  # Assuming positive class probabilities
...
def compute_stability_metrics(features_list):
    ...
    if isinstance(first_element, int):
        r_list_of_lists = ListVector(
            {
                f"dataset_{i+1}": IntVector(inner_list)
                for i, inner_list in enumerate(features_list)
            }
        )
    elif isinstance(first_element, str):
        r_list_of_lists = ListVector(
            {
                f"dataset_{i+1}": StrVector(inner_list)
                for i, inner_list in enumerate(features_list)
            }
        )
    else:
