...
from src.metrics import compute_stability_metrics, compute_performance_metrics
import pytest
import numpy as np
...
def data():
    ...
    num_samples = 1000
    num_features = 10
    X_train = np.random.randn(num_samples, num_features)  # Generate random 2D array (1000x10) for features
    y_train = np.random.randint(0, 2, num_samples)         # Generate random 1D array (1000,) for labels (binary classification)
    X_test = np.random.randn(num_samples, num_features)   # Generate random 2D array (1000x10) for test features
    y_test = np.random.randint(0, 2, num_samples)          # Generate random 1D array (1000,) for test labels (binary classification)
...
def test_compute_performance_metrics_classification(data):
    ...
    for classifier in ['RF', 'naiveBayes', 'bagging']:
        results = compute_performance_metrics(classifier, 'classification', X_train, y_train, X_test, y_test)
...
def test_compute_performance_metrics_regression(data):
    ...
    for classifier in ['RF', 'bagging']:
        results = compute_performance_metrics(classifier, 'regression', X_train, y_train, X_test, y_test)
        assert 'MAE' in results
...
def test_compute_performance_metrics_invalid_classifier(data):
    ...
    with pytest.raises(ValueError, match=r"Invalid classifier name '.*'. Please choose among 'RF', 'naiveBayes', or 'bagging'."):
        compute_performance_metrics('invalid_classifier', 'classification', X_train, y_train, X_test, y_test)
...
def test_compute_performance_metrics_invalid_task(data):
    ...
    for classifier in ['RF', 'naiveBayes', 'bagging']:
        with pytest.raises(ValueError, match=r"Invalid task type"):
            compute_performance_metrics(classifier, 'invalid task', X_train, y_train, X_test, y_test)

def test_compute_performance_metrics_regressionnaives(data):

    X_train, y_train, X_test, y_test = data
    with pytest.raises(ValueError, match=r"Naive Bayes is not suitable for regression task"):
        compute_performance_metrics('naiveBayes', 'regression', X_train, y_train, X_test, y_test)
