...
def test_extract_features(pipeline_instance):
    ...
    headers = ["hello", "world", "this", "is", "a", "test", "of", "the", "emergency", "broadcast"]
...
def test_get_X_Y(pipeline_instance):
    ...
    # random data frame with 10 features and 5 samples and generic names defined in headers
    headers = ["hello", "world", "this", "is", "a", "test", "of", "the", "emergency", "target"]
    data = pd.DataFrame(np.random.randn(5, 10), columns=headers)
    X, y = pipeline_instance._get_X_y(data)
    assert X.shape == (5, 9)
    assert y.shape == (5,)
...
def test_computes_feature(pipeline_instance):
    ...
    all_features = pipeline_instance._compute_features(selected_features_indices, feature_scores)
    ...
    assert len([feature for feature in all_features if feature.selected]) == len(selected_features_indices)
